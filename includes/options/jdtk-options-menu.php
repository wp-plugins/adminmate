 ...
 * @author JD Tool Kit
 */
class JDTK_Options_Menu extends JDTK_Type_UI {
  ...
  public function main_title() {
    echo "<li class='tab main-tab " . self::ID . "'><a href='#" . self::ID . "'>Navigation Menu</a></li>";
  }
  ...
  public function main_content() {
    ...
    array_push($items, array(
      ...
      'class' => "separator-1"
        ...
        'type' => "checkbox_label",
        'title' => __('Do not apply this sorted navigation menu order.', 'jdtk')
        ), 'menu', 'general', 'not_apply_sorted'
      ), $this->inv(self::INV_3, array(
        "type" => "button_confirm",
        "title" => __("Reset Navigation Menu Settings", 'jdtk'),
        "class" => "more-left-1",
        "dialog" => array(
          "title" => __("Warning! Resetting your Navigation Menu options", 'jdtk'),
          "message" => __("Your custom navigation menu options will be deleted.<br>"
            . "Your WordPress navigation menu will be restored to its defaults.", 'jdtk'),
    ...
    foreach ($this->options['menu']['title'] as $key => $new_title) {
      $original_index = $this->find_index_by_slug($this->original_menu, $key);
      array_push($items, $this->invs(array(
          "type" => "checkbox_icon_label_text",
          "title" => ("" === $new_title) ?
            $this->remove_last_number($this->original_menu[$original_index][0]) :
            $new_title,
          "hint" => __("Leave the cell blank to remain the same title.", 'jdtk'),
          "is_separator" => in_array($key, $separators),
          "original" => $this->original_menu[$original_index][6]
